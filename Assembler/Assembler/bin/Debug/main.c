...
int main(int argc, char * argv[]) {
	...
	bool makeMachineReady = false;
	char* writeOutFileName;
	char* sourceFile;
	bool makeDisassembly = false;
	bool compile = false;
	...
	while ((opt=getopt(argc, argv, "hc:r:d:o:m:v")) != -1) {
		switch (opt) {
			...
			case 'r':
				...
				break;
			case 'd':
				makeDisassembly = true;
				sourceFile = optarg;
				disassembly(&fileToCompile);
				break;
			case 'o':
				makeBinary = true;
				writeOutFileName = optarg;
				writeOutFile(&output);
				break;
			case 'm':
				machineReady(&fileToCompile);
				makeMachineReady = true;
				break;
			case 'v':
				verbose = true;
				break;
